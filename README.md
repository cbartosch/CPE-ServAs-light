...
## Current release: v1.27.15

The current application release uses **DvSum CADDI** as the canonical analytics/context product name, targets Python 3.14.7, and requires current Digital Twin runs to identify the `lpr-digital-twin-run-v3-execution-economics` schema. See [`docs/RELEASE_v1.27.15.md`](docs/RELEASE_v1.27.15.md) and [`docs/CURRENT_SCHEMA_RUN_RECOVERY.md`](docs/CURRENT_SCHEMA_RUN_RECOVERY.md). The startup scripts force-recreate the containers and run an in-container connectivity smoke test so stale UI images and incorrect API routes fail before the browser opens.
