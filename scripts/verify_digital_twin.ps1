...
python -c "import json,pathlib; p=list(pathlib.Path('.verify-data').glob('RUN-*/catalog.json')); assert len(p)==1; c=json.loads(p[0].read_text()); assert c['quality']['passed']; assert c['dataset_count']==16; print('smoke quality PASS',c['run_id'])"
