...
## Hotfix5.3 Streamlit run selection

The Streamlit workflow stores the active canonical `run_id` once and propagates it to Predictive Assurance, Customer Care, Data Explorer, Subscriber 360 and Decision Control. If a browser/session restarts without Streamlit session state, the UI queries `GET /api/runs` and adopts the newest persisted run. Predictive and Customer Care actions remain disabled until a valid run is selected or generated.

## Hotfix5.4 unified operator workflow

Operators start the entire demonstration with `docker compose up --build -d` and
open one UI at port 8501. The **Digital Twin / TAO** page owns Generator,
Predictive Assurance, Customer Care, Data Explorer, Subscriber 360 and Decision
Control. A canonical run ID is shared across those tabs. No separate 8502 UI or
second Compose project is required.
