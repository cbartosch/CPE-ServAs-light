...
DVSUM_CADDI_CAPABILITIES: tuple[CaddiCapability, ...] = (
    ...
    CaddiCapability(
        ...
        authoritative_sources=(
            "NEXT/Dvision real-time feed",
            "LLA seven-day history",
        ),
        caddi_role=(
    ...
    CaddiCapability(
        ...
        target_extension=(
            "Add evidence lineage and a link to the assurance episode or incident."
        ),
    ),
    CaddiCapability(
        key="provisioning",
    ...
    CaddiCapability(
        ...
        authority_note=(
            "Plume would remain authoritative for the Wi-Fi telemetry it provides."
        ),
        target_extension=(
...
def caddi_contract() -> dict[str, Any]:
    ...
    return {
        ...
        "naming_note": (
            "DvSum CADDI is the canonical product name used by the LPR demo; "
...
def project_install_assurance_context(
    *,
    ...
    """Create a customer-safe DvSum CADDI/Genesys projection for one install watch.

    The projection carries lineage and authority fields. It is not a live CADDI
